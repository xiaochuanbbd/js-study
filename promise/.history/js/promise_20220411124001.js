...
promise.all返回成功promise组成的数组
promise.race返回是最先成功的promise
...
class myPromise {
	//executor 立即执行的函数 ，在后面会调用，传入定义的reject和resolve
...
//属性：数组的promise都成功了 才返回成功的数组，只要有一个失败或者错误了就返回reject
myPromise.all = function(promissArrs) {
	console.log(promissArrs);
	let arr = [];//定义一个放置promise结果的空数组
	//返回新的promise，必须都成功才返回数组，不然返回第一个触发失败的promise 
